...
% probability of alive cells to begin with
prob = 0.6;
...
% Generating the time length of the game
for i = 1:100
  A = zeros (N+2, N+2);
  for row = 2:N+1
    for col = 2:N+1
        ...
        % diagonal neighbors
        diag = G(row-1, col-1)+G(row-1, col+1)+G(row+1, col-1)+G(row+1, col+1)
        % nearest neighbors
        near = G(row-1, col)+G(row, col-1)+G(row, col+1)+G(row+1, col);
        ...
        % in the next genertion as if repopulation
